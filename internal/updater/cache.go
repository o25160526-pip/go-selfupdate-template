...
func (c Cache) Prune() error {
	...
	for _, entry := range idx.Entries { entries = append(entries, entry) }
	sort.Slice(entries, func(i, j int) bool { return entries[i].LastUsed.After(entries[j].LastUsed) })
	if len(entries) <= keep { return c.save(idx) }
...
func (c Cache) load() (cacheIndex, error) {
	...
	if os.IsNotExist(err) { return idx, nil }
	if err != nil { return idx, err }
	if err := json.Unmarshal(body, &idx); err != nil { return cacheIndex{Entries: map[string]CacheEntry{}}, err }
	if idx.Entries == nil { idx.Entries = map[string]CacheEntry{} }
	return idx, nil
}
