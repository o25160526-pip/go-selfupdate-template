...
func(c Cache)SaveBlobInfo(b BlobInfo)error{i,_:=c.load();if b.Path==""{b.Path=c.BlobPath(b.SHA256)};if st,e:=os.Stat(b.Path);e==nil{if b.Size==0{b.Size=st.Size()}};b.LastUsed=time.Now().UTC();i.Blobs[b.SHA256]=b;e:=i.Entries[b.SHA256];e.SHA256,e.Path,e.Size,e.Verified,e.LastUsed=b.SHA256,b.Path,b.Size,true,b.LastUsed;i.Entries[b.SHA256]=e;return c.save(i)}
...
func(c Cache)LoadReleases(s,ch string)([]Release,string,bool){b,e:=os.ReadFile(filepath.Join(c.metaDir(),s+"-"+ch+".json"));if e!=nil{return nil,"",false};var v releaseCache;if json.Unmarshal(b,&v)!=nil{return nil,"",false};return v.Releases,v.ETag,c.TTL<=0||time.Since(v.Fetched)<=c.TTL}
func(c Cache)SaveReleases(s,ch string,r []Release,etag string)error{if e:=os.MkdirAll(c.metaDir(),0700);e!=nil{return e};b,e:=json.MarshalIndent(releaseCache{Releases:r,ETag:etag,Fetched:time.Now().UTC()},"","  ");if e!=nil{return e};return os.WriteFile(filepath.Join(c.metaDir(),s+"-"+ch+".json"),append(b,'\n'),0600)}
func(c Cache)TouchMeta(s,ch string){r,e,_:=c.LoadReleases(s,ch);_=c.SaveReleases(s,ch,r,e)}
func(c Cache)load()(cacheIndex,error){i:=cacheIndex{Entries:map[string]CacheEntry{},Blobs:map[string]BlobInfo{}};b,e:=os.ReadFile(filepath.Join(c.Dir,"index.json"));if os.IsNotExist(e){return i,nil};if e!=nil{return i,e};if e=json.Unmarshal(b,&i);e!=nil{return i,e};if i.Entries==nil{i.Entries=map[string]CacheEntry{}};if i.Blobs==nil{i.Blobs=map[string]BlobInfo{}};return i,nil}
func(c Cache)save(i cacheIndex)error{if e:=c.Ensure();e!=nil{return e};b,e:=json.MarshalIndent(i,"","  ");if e!=nil{return e};p:=filepath.Join(c.Dir,"index.json");t:=p+".tmp";if e=os.WriteFile(t,append(b,'\n'),0600);e!=nil{return e};return os.Rename(t,p)}
