...
func runtimeEngine() (*updater.Engine, *config.Config, error) {
	cfg, err := config.Load(); if err != nil { return nil, nil, err }
	client := &http.Client{Timeout: cfg.Timeout.Duration()}
	sources, err := updater.NewSources(cfg, client); if err != nil { return nil, nil, err }
...
func updateCommand(args []string) int {
	...
	if err := fs.Parse(args); err != nil { return exitcode.Usage }
	engine, cfg, err := runtimeEngine(); if err != nil { fmt.Fprintln(os.Stderr, err); return exitcode.Usage }
	request := updater.UpdateRequest{CheckOnly: *check}
	if *wanted != "" { v, parseErr := version.Parse(*wanted); if parseErr != nil { fmt.Fprintln(os.Stderr, parseErr); return exitcode.Usage }; request.Version = &v }
	ctx, cancel := context.WithTimeout(context.Background(), cfg.Timeout.Duration()); defer cancel()
	result, err := engine.Update(ctx, request)
	if err != nil { code := exitcode.From(err); if !*silent || code != exitcode.UpToDate { fmt.Fprintln(os.Stderr, err) }; return code }
	if !*silent { if *check { fmt.Printf("available: %s\n", result.Installed) } else { fmt.Printf("updated %s -> %s from %s\n", result.Previous, result.Installed, result.Source) } }
	return exitcode.OK
}
func watchCommand() int {
	engine, cfg, err := runtimeEngine(); if err != nil { fmt.Fprintln(os.Stderr, err); return exitcode.Usage }
	ctx, stop := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM); defer stop()
	err = engine.Watch(ctx, cfg.CheckInterval.Duration(), func(result updater.UpdateResult, err error) { if err == nil && result.Updated { fmt.Printf("updated to %s; restart required\n", result.Installed) } })
	if err == nil || ctx.Err() != nil { return 0 }; fmt.Fprintln(os.Stderr, err); return exitcode.From(err)
}
func trayCommand() int {
	engine, _, err := runtimeEngine(); if err != nil { fmt.Fprintln(os.Stderr, err); return exitcode.Usage }
	ctx, stop := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM); defer stop()
	if err := desktop.Run(ctx, engine); err != nil { fmt.Fprintln(os.Stderr, err); return 1 }; return 0
}
func menuCommand() int {
	...
	for {
		fmt.Println("\nSelf-update menu\n1) Show version\n2) Check for update\n3) Install latest")
		items := features.MenuItems(); for i, item := range items { fmt.Printf("%d) %s\n", 4+i, item.Title) }
		fmt.Println("0) Exit"); fmt.Print("> "); line, _ := reader.ReadString('\n'); choice := strings.TrimSpace(line)
		switch choice {
		case "0": return 0
		...
		default:
			handled := false
			for i, item := range items { if choice == fmt.Sprint(4+i) { handled = true; if err := item.Action(ctx); err != nil { fmt.Fprintln(os.Stderr, err) }; break } }
